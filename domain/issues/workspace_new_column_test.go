package issues_test

import (
	"fickle/domain/issues"
	"fickle/domain/users"
	"fickle/infrastructure/datasource/inmemory"
	"reflect"
	"testing"
)

func TestWorkspace_NewColumn(t *testing.T) {
	type fields struct {
		Id       issues.IdWorkspace
		Name     string
		Columns  []issues.Column
		Archived bool
		UserId   users.IdUser
	}
	type args struct {
		f issues.IFactory
		r issues.IRepository
		p issues.AddBoardColumnParam
	}
	tests := []struct {
		name    string
		fields  fields
		args    args
		want    issues.Column
		wantErr bool
	}{
		{
			name: "ok",
			fields: fields{
				Id:   "1",
				Name: "general",
				Columns: []issues.Column{
					{
						Id:          "1",
						Name:        "Backlog",
						Color:       "#2c3e50",
						Hidden:      false,
						Order:       "1",
						Default:     true,
						WorkspaceId: "1",
						UserId:      "1",
					},
				},
				Archived: false,
				UserId:   "1",
			},
			args: args{
				f: inmemory.NewFactoryIssues(),
				r: inmemory.NewRepositoryIssues(),
				p: issues.AddBoardColumnParam{
					Name:    "Next up",
					Color:   "#f39c12",
					Hidden:  false,
					Order:   "11",
					UserId:  "1",
					Default: false,
				},
			},
			want: issues.Column{
				Name:        "Next up",
				Color:       "#f39c12",
				Hidden:      false,
				Order:       "11",
				Default:     false,
				WorkspaceId: "1",
				UserId:      "1",
			},
			wantErr: false,
		},
		{
			name: "ok",
			fields: fields{
				Id:   "1",
				Name: "general",
				Columns: []issues.Column{
					{
						Id:          "1",
						Name:        "Backlog",
						Color:       "#2c3e50",
						Hidden:      false,
						Order:       "1",
						Default:     true,
						WorkspaceId: "1",
						UserId:      "1",
					},
				},
				Archived: false,
				UserId:   "1",
			},
			args: args{
				f: inmemory.NewFactoryIssues(),
				r: inmemory.NewRepositoryIssues(),
				p: issues.AddBoardColumnParam{
					Name:    "Next up",
					Color:   "rgb(243,156,18)",
					Hidden:  false,
					Order:   "11",
					UserId:  "1",
					Default: false,
				},
			},
			want: issues.Column{
				Name:        "Next up",
				Color:       "#f39c12",
				Hidden:      false,
				Order:       "11",
				Default:     false,
				WorkspaceId: "1",
				UserId:      "1",
			},
			wantErr: false,
		},
		{
			name: "failed to validate: 'Name' empty",
			fields: fields{
				Id:   "1",
				Name: "general",
				Columns: []issues.Column{
					{
						Id:          "1",
						Name:        "Backlog",
						Color:       "#2c3e50",
						Hidden:      false,
						Order:       "1",
						Default:     true,
						WorkspaceId: "1",
						UserId:      "1",
					},
				},
				Archived: false,
				UserId:   "1",
			},
			args: args{
				f: inmemory.NewFactoryIssues(),
				r: inmemory.NewRepositoryIssues(),
				p: issues.AddBoardColumnParam{
					Name:    "",
					Color:   "#f39c12",
					Hidden:  false,
					Order:   "11",
					UserId:  "1",
					Default: false,
				},
			},
			want:    issues.Column{},
			wantErr: true,
		},
		{
			name: "failed to validate: 'Color' empty",
			fields: fields{
				Id:   "1",
				Name: "general",
				Columns: []issues.Column{
					{
						Id:          "1",
						Name:        "Backlog",
						Color:       "",
						Hidden:      false,
						Order:       "1",
						Default:     true,
						WorkspaceId: "1",
						UserId:      "1",
					},
				},
				Archived: false,
				UserId:   "1",
			},
			args: args{
				f: inmemory.NewFactoryIssues(),
				r: inmemory.NewRepositoryIssues(),
				p: issues.AddBoardColumnParam{
					Name:    "Next up",
					Color:   "",
					Hidden:  false,
					Order:   "11",
					UserId:  "1",
					Default: false,
				},
			},
			want:    issues.Column{},
			wantErr: true,
		},
		{
			name: "failed to validate: 'Color' invalid",
			fields: fields{
				Id:   "1",
				Name: "general",
				Columns: []issues.Column{
					{
						Id:          "1",
						Name:        "Backlog",
						Color:       "#",
						Hidden:      false,
						Order:       "1",
						Default:     true,
						WorkspaceId: "1",
						UserId:      "1",
					},
				},
				Archived: false,
				UserId:   "1",
			},
			args: args{
				f: inmemory.NewFactoryIssues(),
				r: inmemory.NewRepositoryIssues(),
				p: issues.AddBoardColumnParam{
					Name:    "Next up",
					Color:   "invalid",
					Hidden:  false,
					Order:   "11",
					UserId:  "1",
					Default: false,
				},
			},
			want:    issues.Column{},
			wantErr: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			w := &issues.Workspace{
				Id:       tt.fields.Id,
				Name:     tt.fields.Name,
				Columns:  tt.fields.Columns,
				Archived: tt.fields.Archived,
				UserId:   tt.fields.UserId,
			}
			got, err := w.NewColumn(tt.args.f, tt.args.r, tt.args.p)
			if (err != nil) != tt.wantErr {
				t.Errorf("Workspace.NewColumn() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if err != nil {
				return
			}
			tt.want.Id = got.Id
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("Workspace.NewColumn() = %v, want %v", got, tt.want)
			}
		})
	}
}
